...
if (if_boundary == T){
  fhat <- ks::kde.boundary(x = X,
                           xmin = c(min(X[, 1]), min(X[, 2])),     # compact support defined by max and min
                           xmax = c(max(X[, 1]), max(X[, 2])),
...
if (if_sobol == T){
  # Draw Sobol points from the support
  library(qrng)
  M            <- 1000000
  ...
  x1           <- seq(min(X[,1]), max(X[,1]), length.out = 1000) 
  x2           <- seq(min(X[,2]), max(X[,2]), length.out = 1000)
  G            <- expand.grid(x1, x2)
  ...
  fx_norm      <- fx / sum(fx)
  M            <- 1000000
  idx          <- sample(seq_along(fx_norm), size = M, replace = TRUE, prob = fx_norm)
  X_samp       <- G[idx, ]
...
# OLS formula
library(Matrix)
# B      <- bdiag(Psi_X_trim.t, Psi_X_trim.c)
# B      <- as.matrix(B) 
