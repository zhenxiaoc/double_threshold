...
resid    <- ifelse(df$D == 1, df$earnings - mu0_X.t, df$earnings - mu0_X.c)
lambda2  <- 1
asymp_var_hat  <- var(pmax(mu0_X.t - mu0_X.c, 0)) + mean(ind * lambda2 * resid^2 / (p_hat * (1 - p_hat)))
sd_hat   <- sqrt(asymp_var_hat/nrow(df))
