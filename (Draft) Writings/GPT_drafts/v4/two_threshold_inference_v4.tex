...
and we show that the margin bound, unlike the corner bound, is exhibited as
attained only in mesh-resonant examples, its exact order in general remaining
open. The accounting implies
...
Assumption~\ref{ass:rem} collects them --- and
...
hypotheses but not as to their proofs.
...
\emph{second-derivative modulus} condition, Assumption~\ref{ass:rem}(a),
which has the further advantage of not presupposing that $D^{3}\Th$ exists
--- the segment from $\gamma_{0}$ to $\bar\gamma$ passes through the truth,
which is only $C^{2}$.
...
nesting argument.
(c) (\emph{Corner noncancellation.}) Let
...
$\mathcal V_{\bullet,n}/K_{n}$. Either $\Corner=\emptyset$, or
\begin{equation}
\text{(c1)}\quad
...
Both are needed only for lower bounds; no upper bound below uses either.
\end{assumption}
...
\item (\emph{Second-derivative modulus.}) There is a nondecreasing
$\varpi$ with $\varpi(0+)=0$ such that, on a neighbourhood of $\gamma_{0}$
containing the estimated paths w.p.\ $\to1$,
\begin{equation}
\big|\{D^{2}\Th(\gamma)-D^{2}\Th(\gamma_{0})\}[\eta,\eta]\big|
\;\leq\;\varpi\big(\norm{\gamma-\gamma_{0}}_{C^{1}}\big)\,
\norm{\eta}_{\infty}\norm{\eta}_{C^{1}} .
\label{eq:modulus}
\end{equation}
\item (\emph{Taylor remainder.}) With $R_{3}:=\tfrac12\{D^{2}\Th(\gamma_{\xi})
...
Three comments on the formulation. First, (a) deliberately avoids
postulating a \emph{third} shape derivative. The segment from $\gamma_{0}$
to $\bar\gamma$ passes through the truth, which Assumption~\ref{ass:geometry}
gives only as $C^{2}$; a third-order Taylor expansion around $\gamma_{0}$
would need $C^{3}$ there, contradicting the H\"older parameter space
whenever $s<3$. The mean-value form of (b) needs only $D^{2}$ plus the
modulus (a). Second, (a) also controls the SS \emph{evaluation-point}
remainder: the estimator subtracts $D^{2}\Th(\bar\gamma)[\Delta,\Delta]$
whereas the cancellation argument uses $D^{2}\Th(\gamma_{0})$, and the
difference is bounded by the same $\varpi$. Third, the strength of (b) is
entirely inherited from $\varpi$; Remark~\ref{rem:R3} converts each candidate
modulus into a smoothness requirement.
...
Assumption~\ref{ass:rem}(b)--(c) in their $O_{p}(r_{n}^{*})$ form. Then
...
\item (\emph{Centered CLT.}) Impose Assumption~\ref{ass:rem}(b)--(d) in
...
Assumption~\ref{ass:rem}(b)--(c); v3 omitted both from this part of the
statement.
...
Lyapunov condition Assumption~\ref{ass:rem}(d) --- which v3 invoked from
...
(c$'$) the four \emph{arm} residuals used in \eqref{eq:two-band-var} are
consistent, $n^{-1}\sum_{i}\{\hat\epsilon_{o,i}-\epsilon_{o,i}\}^{2}
=o_{p}(1)$ for $o\in\{S,Y\}$ --- consistency of the \emph{contrasts}
$\hat g,\hat h$ does not imply it, since a common additive error in
$\hat\mu_{o}(\cdot,1)$ and $\hat\mu_{o}(\cdot,0)$ cancels from the contrast
but not from the residuals;
...
Assumption~\ref{ass:nondegen}(c) is imposed on the normalized sequence
rather than derived from $\rho_{SY}\neq0$. The stochastic orders below
...
\end{equation}
where $V_{g}:=\E[\Delta_{g}^{2}]=\mathcal V_{gg,n}/n+b_{g}^{2}$,
$V_{h}$ likewise, and $V_{gh}:=\E[\Delta_{g}\Delta_{h}]
=\mathcal V_{gh,n}/n+b_{g}b_{h}$, all at $x$. All three are $O(K_{n}/n)$ up
to their bias parts, so $\mathcal B_{\Corner,n}=O(K_{n}/n)$ --- and
...
result]\label{thm:ss}
Let Assumptions~\ref{ass:model}--\ref{ass:rem} and \ref{ass:path} hold, let
...
\label{eq:R3SS}
\end{equation}
both terms being bounded by the modulus \eqref{eq:modulus}. Then:
\begin{enumerate}[leftmargin=2em,label=(\roman*)]
...
\item (\emph{Centered CLT, jackknife estimator.}) For the tuning-free
$\hat\Th_{SS}^{\,\mathrm{jack}}$ of \eqref{eq:jackknife}, the fourth-order
error of Lemma~\ref{lem:jackknife} must \emph{also} be
$o(\mathrm{se}_{n})$, which under Assumption~\ref{ass:path}(d) requires its
own sieve window:
\begin{equation}
...
remainder. Both are now bounded by the single modulus \eqref{eq:modulus}.
\end{remark}

\begin{remark}[$R_{3}^{SS}$ is an assumption, not a consequence]
\label{rem:R3}
Assumption~\ref{ass:rem}(a)--(b) is where the weak-smoothness claim actually
rests, and we have not verified it from primitive conditions. What can be
said is how restrictive it is under each candidate modulus $\varpi$.
Substituting $\norm{\bar\Delta}_{\infty}\asymp\sqrt{K_{n}/n}$ and
$\norm{\bar\Delta}_{C^{1}}\asymp K_{n}^{1/2+1/d}n^{-1/2}$ at $a=d/(2s+1)$
into $R_{3}^{SS}\lesssim
\varpi(\norm{\bar\Delta}_{C^{1}})\norm{\bar\Delta}_{\infty}
\norm{\bar\Delta}_{C^{1}}$:
\[
\begin{array}{lll}
\varpi(\epsilon)\asymp\epsilon\ \text{in }\norm{\cdot}_{C^{1}}
 &\text{(2 gradients)} &\Longrightarrow\ s>\tfrac{3d+1}{4},\\[2pt]
\varpi(\epsilon)\asymp\epsilon\ \text{in }\norm{\cdot}_{\infty}
 &\text{(1 gradient)} &\Longrightarrow\ s>\tfrac{3d-1}{4},
\end{array}
\]
against the crude $R_{3}\lesssim\norm{\bar\Delta}_{C^{1}}^{3}$ of v3, which
gives $s>3(d+1)/4$. Only the last is weaker than $(d+1)/2$ at $d=2$ ($1.25$
against $1.5$). Since $D^{2}\Th$ in \eqref{eq:D2}--\eqref{eq:Qg} carries
exactly one gradient across its two directions, the one-gradient modulus is
the plausible one, and the same conjecture would close the jackknife gap of
Remark~\ref{rem:jackgap}. Establishing it requires a Lipschitz bound on
$\gamma\mapsto D^{2}\Th(\gamma)$ --- which, unlike a third shape derivative,
does \emph{not} require extra smoothness of $\gamma_{0}$ and is therefore
compatible with the H\"older parameter space for every $s>1$. That is the
principal technical task left open by this draft. Until it is done,
Theorem~\ref{thm:ss} is conditional: $s>(d+1)/2$ is the range on which
\emph{every other} term in the error budget is controlled, and
Assumption~\ref{ass:rem}(b) is imposed on that range.
\end{remark}
...
Assumption~\ref{ass:rem}(b) --- see Remark~\ref{rem:R3} --- the
equicontinuity term of Assumption~\ref{ass:rem}(c), and, for the jackknife
...
$\mathcal B^{\times,\mathrm{stoch}}_{\Corner,n}$ only: because the common
...
is $o(r_{n}^{*})$ and is removed by undersmoothing, not by debiasing.
Consequently, \emph{provided} Assumption~\ref{ass:nondegen}(c2) holds ---
...
(g) the Lyapunov condition Assumption~
ef{ass:rem}(d) holds, and
...
case $\Th(\cdot)$ is itself a sample average, so one must say over which
observations. We evaluate every $\Th(\cdot)$ inside $\Psi$ over the
\emph{full} sample. The resulting discrepancy from the population $\Th$ is
the equicontinuity term of Assumption~\ref{ass:rem}(c), already assumed
$o_{p}(\mathrm{se}_{n})$; it is common to all three evaluations in $\Psi$
and therefore does not disturb the cancellation in \eqref{eq:dd-leftover},
but it is \emph{not} negligible by fiat and is the reason (c) is stated for
every estimator in this paper rather than for the plug-in alone. In the
known-density case the issue does not arise. \emph{Representer conditions.}
...
are band constructions at $C^{1}$-close level sets, the conditions transfer,
but they are conditions on a \emph{difference} of band vectors and we state
them as such rather than inheriting them.
...
The variance estimator must be formed from the \emph{actual} composite score:
$\hat\sigma_{n}^{2}=\tfrac3n\sum_{i\in C}\{\sum_{o}\sum_{j}
\hat v^{*j}_{o}(X_{i},W_{i})\hat\epsilon^{j}_{o,i}\}^{2}$, averaged over the
three assignments --- not the ordinary two-band score of
\eqref{eq:two-band-var}, unless the two are shown to be ratio-equivalent.
(They should be, since $\hat v^{*A}+\hat v^{*B}\approx v^{*}_{K_{n}}$, but we
do not prove it.)
...
stochastic diagonal, $\asymp K_{n}^{1+1/d}/n$ for a sieve --- by
...
\item \textbf{The second-derivative modulus \eqref{eq:modulus}.}
Assumption~\ref{ass:rem}(a)--(b), the third-order half of
Assumption~\ref{ass:dml}(f), and condition (iii) of Theorem~\ref{thm:dd} all
rest on a Lipschitz-type bound for $\gamma\mapsto D^{2}\Th(\gamma)$ that we
have not established. Remark~\ref{rem:R3} converts each candidate modulus
into a smoothness requirement: the one-gradient modulus gives
$s>(3d-1)/4$, weaker than $(d+1)/2$ at $d=2$; the two-gradient modulus gives
$s>(3d+1)/4$; v3's crude $\norm{\bar\Delta}_{C^{1}}^{3}$ gives
$s>3(d+1)/4$. This formulation is deliberately weaker than postulating
$D^{3}\Th$, which would require $\gamma_{0}\in C^{3}$ and so contradict the
parameter space for $s<3$. The same bound would settle the jackknife gap of
Remark~\ref{rem:jackgap}. \emph{This is the single most important open item
in the paper.}
...
$K_{n}^{1+1/d}/n$ is attained, and generic examples in which the integral is
an order smaller. The exact order for curved margins, general $d$, and
heteroskedastic errors is open; settling it would sharpen (or weaken) the
$s\geq d$ of Theorem~\ref{thm:plugin}(i). Until then $K_{n}^{1+1/d}/n$ is
used only as an upper bound.
\item \textbf{Stochastic equicontinuity.} Assumption~\ref{ass:rem}(c) is
...
the ordinary case, and the ratio-equivalence of the composite-score variance
estimator to \eqref{eq:two-band-var} is asserted, not proved.
\end{enumerate}
