...
spec <-   list(
  ...
  rF   = function(m) data.frame(X1 = runif(m, -1.5, 1.5), X2 = runif(m, -1.5, 1.5)), 
...
ite            <- 2000        # number of iterations
M              <- 5000        # number of sobol points to estimate W_0
alpha          <- 0.05
basis          <- "tensor"    # c("tensor","additive","glp")
J.x.degree     <- 3           # cubic spline
J.x.segments.t <- 4           # number of segments to estimate mu_0(x,1)
J.x.segments.c <- 1           # number of segments to estimate mu_0(x,0)
knots          <- "uniform" # c("uniform","quantiles")
X.min          <- NULL
X.max          <- NULL
...
for (n in ns) {
  ...
  one_ite <- function(dummy) {
    ...
    W_hat_i     <- 3^2 * mean((mu0_int_X.t - mu0_int_X.c >= 0)) # W_0 estimate
    ...
    Bun    <- 3^2 * colMeans(term_1 * term_3)/(2*eps)
    ...
    return(c(W_hat = W_hat_i, se = se_i, num = num_i))
  }
  ...
  final_df <- rbind(final_df, data.frame(
    ...
    se        = mean(se),
    sd_se     = sd(se),
    coverage  = mean(abs(W_hat - pi)/se <= 1.96)
  ))
  ...
  cat("spec =", spec$name, " n =", n,
      ...
      " | sd =", round(sd(W_hat), 3),
      " | se =", round(mean(se), 3),
      " | coverage =", round(mean(abs(W_hat - pi)/se <= 1.96), 3),"\n")
}

print(final_df)

save(final_df, res_mat, file = "sim_results_Thm4.Rdata")
