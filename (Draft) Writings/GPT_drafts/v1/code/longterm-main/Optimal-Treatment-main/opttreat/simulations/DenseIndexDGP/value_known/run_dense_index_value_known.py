"""DenseIndexDGP known-distribution value/share simulation, RF first stage.

Self-contained runner in the style of the CCG 2025
``ccg_value_known/run_ccg_value_known.py`` script: the DGP, the RF-sieve tuning,
and the simulation settings are top-level constants, and the Monte Carlo loop is
spelled out in ``run``. The random-feature sieve map is built by the ``rf_ridge``
estimator directly from the quasi-sphere options, so there is no custom
feature-map helper; the first-stage estimator, the target parameter, and the
SieveVar standard error come straight from ``opttreat.estimation``,
``opttreat.parameters`` and ``opttreat.variance``.

The target is the known-distribution value/share V0 = E[1{tau(X) > 0} m(X)] with
m(x) = 1, i.e. the positive treatment-effect probability over Uniform([0, 1]^d).
Configuration is the best-coverage DenseIndexDGP value design from the existing
simulation results: 200 quasi-sphere sigmoid random features, with a large
variance Sobol grid so the eps-band value derivative is populated, and the
leave-one-out debiased estimator approaching ~0.95 coverage at the largest sample
...
# ---------------- RF-sieve first stage (best value design) ----------------
N_FEATURES = 200
...
VALUE_IOTA = 0.01  # eps = iota * sd(h_hat) for the value-derivative band

STEM = "dense_index_value_known"
OUTPUT_DIR = Path(__file__).resolve().parent / "results"


def v_weight(x: np.ndarray) -> np.ndarray:
    """Unit value function v(x) = 1 (the density f(x) defaults to 1)."""
