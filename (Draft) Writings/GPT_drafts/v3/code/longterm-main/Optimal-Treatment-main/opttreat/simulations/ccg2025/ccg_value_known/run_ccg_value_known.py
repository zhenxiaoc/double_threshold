"""CCG 2025 Theorem 3 known-distribution value-functional simulation (Model 15).

Self-contained runner in the style of the R ``TEST_Thm3_Spline_SieveVar.R``
script. The plug-in estimate, LOO-debiased estimate, and SieveVar standard error
of the value functional E[1{h(X) > 0} v(X)] (with v(X) = 9, true value pi) come
...
NS = (1500, 3000, 6000)
THETA_SOBOL = 5000
VALUE_VARIANCE_SOBOL = 1_000_000
VALUE_SCALE = 9.0
VALUE_EPS = 0.005
SEED = 2025
JOBS = 3
...
STEM = "ccg_value_known"
OUTPUT_DIR = Path(__file__).resolve().parent / "results"


def run(specs=SPECS, ns=NS, ite=ITE, *, jobs=JOBS, theta_sobol=THETA_SOBOL,
        value_variance_sobol=VALUE_VARIANCE_SOBOL, output_dir=OUTPUT_DIR):
    def value_weight(X):
        """Target-measure weight v(X) = VALUE_SCALE."""
        return np.full(np.asarray(X).shape[0], VALUE_SCALE)
    ...
    for s_idx, spec in enumerate(specs):
        ...
        # Target parameter: value functional E[1{h(X) > 0} v(X)] (known dist).
        param = get_parameter(ParameterConfig("value_known", options={
            "dim": model.dim,
        ...
        # SieveVar standard error for the known-distribution value functional.
        variance = get_variance_estimator(VarianceConfig(method="sieve_var", options={
            "param_type": "value_known",
            "dim": model.dim,
            "n_sobol": int(value_variance_sobol),
            "transform": model.inverse_CDF,
            "sobol_scramble": False,
            "v_func": value_weight,
            "loo_eps": VALUE_EPS,
...
def smoke_main(output_dir=OUTPUT_DIR):
    """Tiny local check over the value model."""
    return run(specs=[SPECS[0]], ns=(90,), ite=1, jobs=1, theta_sobol=512, value_variance_sobol=64, output_dir=output_dir)
