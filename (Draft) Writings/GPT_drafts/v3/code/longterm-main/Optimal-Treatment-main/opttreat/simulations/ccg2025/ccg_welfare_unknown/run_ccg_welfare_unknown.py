"""CCG 2025 Theorem 2 unknown/common-support welfare simulation (Models 8-14).

Self-contained runner in the style of the R ``TEST_Thm2_*_Spline_SieveVar.R``
scripts. The plug-in estimate, LOO-debiased estimate, and SieveVar standard
error of the empirical common-support welfare n^{-1} sum_i max(h(X_i), 0) are
evaluated on the treated/control common-support rows of each draw, using the
estimator/parameter/variance kernels from ``opttreat`` directly.

Set ``ESTIMATOR`` to ``"sieve"`` or ``"rf_ridge"`` (both return the same
estimator-output structure). LOO debiasing defaults to the analytical
boundary-band method (``LOO_METHOD = "band"``).
"""
...
from opttreat.data import common_support_mask, ensure_2d_features, split_treated_control
...
SPECS = [
    {"model": "Model8", "Jt": 8, "Jc": 8},
    {"model": "Model9", "Jt": 4, "Jc": 1},
    {"model": "Model10", "Jt": 1, "Jc": 4},
    {"model": "Model11", "Jt": 1, "Jc": 1},
    {"model": "Model12", "Jt": 1, "Jc": 1},
    {"model": "Model13", "Jt": 1, "Jc": 1},
    {"model": "Model14", "Jt": 1, "Jc": 1},
]
...
STEM = "ccg_welfare_unknown"
OUTPUT_DIR = Path(__file__).resolve().parent / "results"


def run(specs=SPECS, ns=NS, ite=ITE, *, jobs=JOBS, theta_sobol=THETA_SOBOL, output_dir=OUTPUT_DIR):
    draw_rows = []
    for s_idx, spec in enumerate(specs):
        ...
        # Target parameter: empirical common-support welfare n^{-1} sum max(h, 0).
        param = get_parameter(ParameterConfig("welfare_unknown", options={
            "dim": model.dim,
        ...
        # SieveVar standard error for the unknown/common-support welfare functional.
        ...
        for n_idx, n in enumerate(ns):
            ...
            def one_ite(seed_i, n=n):
                ...
                # Covariate rows inside the treated/control common support.
                X_all = ensure_2d_features(df.filter(like="X").to_numpy(), name="X")
                d = np.asarray(df["d"], dtype=float).reshape(-1)
                mask = common_support_mask(X_all, d, strict=True)
                if mask.sum() == 0:
                    raise ValueError("common support is empty for this draw.")
                X_eval = X_all[mask]

                W_plug = float(param.plug_in(output["h_hat"], X_eval))
                W_loo = float(param.loo(output, X_eval, delta0=LOO_DELTA0))
                out_var = dict(output)
                out_var["X_eval"] = X_eval
                se = float(np.sqrt(max(variance.fit(out_var), 0.0)))
...
def smoke_main(output_dir=OUTPUT_DIR):
    ...
    return run(specs=[SPECS[0]], ns=(90,), ite=1, jobs=1, theta_sobol=512, output_dir=output_dir)
