...
pathwise derivative is a sum of two boundary (Hausdorff) integrals over the
...
we show that the margin bound --- unlike the corner bound --- is attained
only when the margin resonates with the sieve mesh. The accounting implies
that margin-by-margin quadratic debiasing \`{a} la \citet{cg2026} leaves a
corner cross term that is rate-relevant on the window $(d+1)/2<s\leq d-1$,
nonempty for $d\geq4$; we therefore construct corner-aware split-sample and
...
explicit about what is and is not proved. Under a third-order remainder
bound we do not establish, the corner-aware split-sample estimator attains
$n^{-s/(2s+1)}$ under $s>(d+1)/2$; under the crude bound available to us the
range shrinks to $s>3(d+1)/4$. A centered normal limit requires strict
...
\paragraph{Status of the results.} Theorem~\ref{thm:D2} (the second
derivative), Proposition~\ref{prop:D1}, the polarization identity
\eqref{eq:polar-limit}, the support-edge term of Remark~\ref{rem:edge}, and
...
displays the remainder conditions it needs, and
Appendix~\ref{app:proofs}, \S B.4 lists the four that we have not verified
from primitive conditions. This is a working draft; the inferential
statements should be read as a specification of what must be proved, not as
proved results.

\paragraph{Revision note (v3).} Relative to v2: \emph{(i)} The doubly
debiased estimator of v1--v2 is \emph{invalid}. Adding fold-specific one-step
Riesz corrections to the split-sample combination reintroduces the own-half
second-order diagonals that the quadratic correction removes, with a factor
$-\tfrac12$; Remark~\ref{rem:ddbug} gives the counterexample and
Appendix~\ref{app:numeric}, item~7, the numerical demonstration.
Section~\ref{sec:dd} derives the score of the composite split-sample
functional and gives a corrected three-fold construction.
\emph{(ii)} Theorem~\ref{thm:plugin}'s asymptotic normality claim is split
into a rate statement (valid at the rate-optimal $K_{n}$ under $s\geq d$) and
a centered-CLT statement, which requires strict undersmoothing and strict
$s>d$; at $s=d$ the quadratic diagonal is $O(r_{n}^{*})$, not $o(r_{n}^{*})$,
and the sieve approximation bias is of the same order as the standard
deviation. The analogous window is given for Theorem~\ref{thm:ss}.
\emph{(iii)} Theorem~\ref{thm:ss} is restated as conditional on a third-order
remainder bound, with the ladder of smoothness requirements each candidate
bound implies; the crude bound gives $s>3(d+1)/4$, not $s>(d+1)/2$.
\emph{(iv)} Assumption~\ref{ass:path} no longer requires $\gamma_{0}\in
C^{3}$, which contradicted the H\"older parameter space; the smoothness is
now imposed on $w$ ($C^{3}$) and on the \emph{sieve space} ($C^{4}$, i.e.\
B-spline degree $\geq5$), following the derivative count of
Appendix~\ref{app:D2}. The cubic splines of Section~\ref{sec:mc} are only
$C^{2}$ and do not suffice for the jackknife bound.
\emph{(v)} Assumption~\ref{ass:dml}(c) required a Hausdorff-measure symmetric
difference of two hypersurfaces to vanish, which is impossible for distinct
surfaces; it is replaced by Hausdorff-distance and region-measure conditions,
for both $g$ and $h$. Assumption~\ref{ass:dml}(f) bounded a quadratic
remainder by a cubic quantity; it is replaced by
$\norm{\Delta}_{\infty}\norm{\Delta}_{C^{1}}+\norm{\Delta}_{\infty}^{2}$
plus a separate third-order condition. Representer consistency
$\norm{\hat v^{*}-v^{*}}_{sd}=o_{p}(\sigma_{n})$ is added, since the product
rate alone does not deliver a consistent variance.
\emph{(vi)} The pointwise second-moment identities of \eqref{eq:pointwise}
are restated in exact sandwich form, with the local approximation and its
error made explicit; they are not exact under heteroskedasticity.
\emph{(vii)} The corner term is $O(K_{n}/n)$; a matching lower bound requires
the noncancellation condition now imposed in
Assumption~\ref{ass:nondegen}(c), since the corner coefficient is signed and
can cancel across terms and across corner points.
\emph{(viii)} The parameter space $\Lambda^{s}_{c}$ and the nondegeneracy
conditions needed for $\sigma_{n}^{2}\asymp K_{n}^{1/d}$ and for the minimax
lower bound are now defined (Assumption~\ref{ass:nondegen}). Note that
Assumption~\ref{ass:nondegen}(b) requires only \emph{one} nondegenerate
margin: demanding two would exclude the single-threshold submodel on which
the minimax lower bound of Theorem~\ref{thm:plugin}(ii) is built.
\emph{(ix)} Remark~\ref{rem:biasprod} records what no split-sample device
repairs: $\E\,D^{2}\Th[e_{A},e_{B}]$ is the \emph{bias product}
$D^{2}\Th[b_{A},b_{B}]$, which is $o(r_{n}^{*})$ for undersmoothed sieves but
not for a systematically biased machine-learning first stage.
\emph{(x)} Section~\ref{sec:offdiag} is new. It extracts from the existing
Monte Carlo table a parameter-free test of the cross-half off-diagonal order
of Appendix~\ref{app:proofs}, \S B.2, and shows that the SS estimator's
standard-error deficit is entirely the variance of the quadratic correction
--- the finite-sample counterpart of the upper edge of
\eqref{eq:ss-window}.
Relative to v1, the corrections recorded in the v2 revision note stand: the
margin/corner order distinction, the corner coefficient, and the two
incorrect claims of Remarks~\ref{rem:ls} and~\ref{rem:outofspan}.

The Monte Carlo tables are reproduced unchanged from v1;
Section~\ref{sec:limits} states which of them do and do not support the
asymptotic claims, and now includes the doubly debiased construction among
the items requiring repair.
...
The remaining maintained conditions are nondegeneracy requirements. They are
separated out because several statements below are two-sided (``$\asymp$'',
``rate-relevant'', ``minimax'') and are false without them: a functional with
$v_{0}\equiv0$, or with empty margins, or whose corner coefficient cancels
exactly, satisfies Assumptions~\ref{ass:model}--\ref{ass:geometry} and has no
irregularity at all.
...
&\text{Assumptions~\ref{ass:model}--\ref{ass:geometry} and (b)--(c) below
...
nesting argument.
(c) (\emph{Corner noncancellation.}) Either $\Corner=\emptyset$, or
\[
\Big|\int_{\Corner}\frac{w}{2\abs{\sin\omega}}
\Big[\frac{2\varrho_{gh}}{\Ng\Nh}
-\cos\omega\Big(\frac{\varrho_{gg}}{\Ng^{2}}
+\frac{\varrho_{hh}}{\Nh^{2}}\Big)\Big]\dd\Haus^{d-2}\Big|
\;\geq\;\underline c_{\Corner}>0,
\]
where $(\varrho_{gg},\varrho_{hh},\varrho_{gh})$ are the arm-aggregated
conditional second moments of the two contrast regressions, defined in
\eqref{eq:aggmoments} below. The bracket is \emph{signed}: the within-unit covariance term
and the two $\cos\omega$-weighted variance terms may offset each other, and
contributions from different corner points may offset. Condition (c) rules
this out; it is needed only for the lower-bound half of the corner
statements (Proposition~\ref{prop:margins}), never for the upper bounds.
\end{assumption}

\begin{remark}[$\Corner$ is not automatically material]\label{rem:cornerzero}
Assumption~\ref{ass:nondegen}(c) is a genuine restriction, not a formality.
At an orthogonal corner ($\cos\omega=0$) with $\rho_{SY}=0$ the bracket
vanishes identically; and with $d=2$ and several corner points the terms
carry the sign of $\cos\omega$ at each point, which typically varies. In the
...
\item (\emph{Rate.}) At $a=d/(2s+1)$ and $s\geq d$,
$\hat\Th-\Th_{0}=O_{p}(r_{n}^{*})$, $r_{n}^{*}=n^{-s/(2s+1)}$, with a
constant depending on $\mathcal P^{s}_{c}$ only through
$(c,\underline c,\underline c_{T},\underline w,\underline m)$ and the sieve
constants --- hence uniformly over $\mathcal P^{s}_{c}$, \emph{provided} the
cited sieve regularity conditions hold uniformly over the class, which we
assume but have not verified.
\item (\emph{Minimax.}) $r_{n}^{*}$ is a minimax lower-bound rate over
$\mathcal P^{s}_{c}$, so the plug-in rate in (i) is sharp.
\item (\emph{Centered CLT.}) Suppose in addition that
$R_{3}=o_{p}(\sqrt{K_{n}^{1/d}/n})$ and that
\begin{equation}
\frac{d}{2s+1}\;<\;a\;<\;\frac{d}{2d+1},
\label{eq:plugin-window}
\end{equation}
a window that is \emph{nonempty if and only if $s>d$ strictly}. Then
$\sqrt n(\hat\Th-\Th_{0})/\sigma_{n}\overset{d}{\to}\mathcal N(0,1)$, and the
attained rate is $\sqrt{K_{n}^{1/d}/n}=n^{-(1-a/d)/2}$, slower than
...
\emph{one} margin be nondegenerate and (c) permits $\Corner=\emptyset$.
...
Lyapunov condition of Assumption~\ref{ass:dml}(g). For the limit to be
\emph{centered}, both the linear bias and the quadratic diagonal must be
$o(\sqrt{K_{n}^{1/d}/n})$: $K_{n}^{-s/d}=o(\sqrt{K_{n}^{1/d}/n})$ iff
$a>d/(2s+1)$, and $K_{n}^{1+1/d}/n=o(\sqrt{K_{n}^{1/d}/n})$ iff
$a<d/(2d+1)$. The two are compatible iff $2s+1>2d+1$.
\end{proof}
...
Second, at $s=d$ the margin quadratic diagonal $K_{n}^{1+1/d}/n$ is
\emph{exactly} $\asymp r_{n}^{*}$, hence $O(r_{n}^{*})$ but not
$o(r_{n}^{*})$: enough for the rate statement (i), not for a centered limit.
...
Let Assumptions~\ref{ass:model}--\ref{ass:nondegen} hold in the
empirical-measure case, and suppose:
...
\begin{equation}
\mathcal V(x):=b(x)'G^{-1}b(x)\asymp K_{n},
\qquad
\norm{G^{-1/2}\nabla b(x)}\lesssim K_{n}^{1/2+1/d},
\label{eq:sieve-scaling}
\end{equation}
(each derivative of a basis function costs a factor $\varkappa^{-1}
=K_{n}^{1/d}$), whence $\abs{b(x)'G^{-1}\partial_{j}b(x)}\lesssim
...
Write the arm-aggregated conditional second moments of the two contrast
regressions as
\begin{equation}
\varrho_{gg}(x):=\sum_{w\in\{0,1\}}
...
\label{eq:aggmoments}
\end{equation}
and $\varrho_{hh}$ symmetrically. The \emph{exact} second moments are
sandwich forms,
\begin{equation}
\E[u_{g}(x)u_{h}(x)]=\frac1n\,b(x)'G^{-1}\Omega_{gh}G^{-1}b(x),
\qquad
\Omega_{gh}:=\E\big[b(X_{i})b(X_{i})'\,\varrho_{gh}(X_{i})\big],
\label{eq:sandwich}
\end{equation}
and similarly for $\E[u_{g}^{2}]$ with $\Omega_{gg}$. Because $b$ is locally
supported, for Lipschitz $\varrho$ these localize:
\begin{align}
\E[u_{g}(x)^{2}]&=\frac{\varrho_{gg}(x)}{n}\,\mathcal V(x)\,(1+O(\varkappa)),
&
\big|\E[u_{g}(x)\,(n_{g}\!\cdot\!\nabla u_{g})(x)]\big|
&\lesssim\frac{\norm{\varrho_{gg}}_{\infty}}{n}\,K_{n}^{1+1/d},
\nonumber\\
\E[u_{g}(x)u_{h}(x)]&=\frac{\varrho_{gh}(x)}{n}\,\mathcal V(x)\,
(1+O(\varkappa)),
&&
\label{eq:pointwise}
\end{align}
the $O(\varkappa)$ carrying a $\mathrm{Lip}(\varrho)/\inf\varrho$ constant
and vanishing identically under conditional homoskedasticity, where
$\Omega_{gh}=\varrho_{gh}G$ exactly.

Two corrections to v2 are recorded here. First, v1 wrote the third display
with an undefined normalization and with ``$\asymp$'' applied to a signed
quantity; v2 replaced it by an \emph{exact} identity
$\E[u_{g}u_{h}]=\varrho_{gh}(x)\mathcal V(x)/n$, which is also wrong: under
heteroskedasticity only \eqref{eq:sandwich} is exact, and the localization is
an approximation. Second, the consequences matter. The sign of
$\E[u_{g}u_{h}](x)$ need not equal that of $\rho_{SY}(x,w)$ at $x$; and a
nonzero conditional correlation at a corner does not by itself guarantee a
nonzero \emph{integrated} corner bias --- which is why
Assumption~\ref{ass:nondegen}(c) is imposed separately rather than derived
from $\rho_{SY}\neq0$. The stochastic orders below survive as upper bounds;
the constants and the ``exact order'' readings do not.
...
$O(K_{n}/n)$ up to their bias parts, so
$\mathcal B_{\Corner}=O(K_{n}/n)$ --- and \emph{only} $O$: the bracket is a
\emph{signed} difference, so the within-unit covariance term and the two
$\cos\omega$-weighted variance terms may offset each other, and
contributions from different corner points may offset. A matching lower
bound is exactly Assumption~\ref{ass:nondegen}(c), which is why that
condition is imposed rather than derived. v2 wrote
``$\mathcal B_{\Corner}\asymp K_{n}/n$''; that is a two-sided claim the
formula does not support on its own.
...
fact $K_{n}/n$, the rate condition relaxes to $s\geq d-1$ and the
...
there is no oscillating derivative to average away, and each of the three is
$\asymp K_{n}/n$ individually. This does \emph{not} make
...
phenomenon, and which Assumption~\ref{ass:nondegen}(c) excludes by
...
margin-versus-corner comparison is an inequality: the corner is never more
than $K_{n}^{1/d}$ smaller than the margins, and on a generic oblique
geometry the two are plausibly of the same order --- which is where v1's
instinct, though not its argument, pointed.
...
one, every $\gamma$ on the segment $\{\bar\gamma+t\Delta:\abs t\leq\tfrac12\}$
satisfies Assumption~\ref{ass:geometry}(a)--(d) with constants
$\underline c/2,\underline c_{T}/2$.
(d) (\emph{Fourth-derivative bound.}) On that segment
$t\mapsto\Th(\bar\gamma+t\Delta)$ is four times continuously differentiable
with $\sup_{\abs t\leq1/2}\abs{\frac{\dd^{4}}{\dd t^{4}}
\Th(\bar\gamma+t\Delta)}\lesssim\norm{\Delta}_{C^{1}}^{4}$.
\end{assumption}

Three comments. First, v2 stated (a)--(d) as a single condition requiring a
...
inconsistency. Second, (b) is a real implementation constraint that v2
...
Carlo uses cubics and therefore satisfies neither.
Third, (c) holds for sieve first stages on an event of probability tending
to one whenever $\norm{\hat\gamma^{j}-\gamma_{0}}_{C^{1}}=o_{p}(1)$, i.e.\
$K_{n}^{1/2+1/d}n^{-1/2}+K_{n}^{-(s-1)/d}=o(1)$, which needs $s>1$ and is
implied by \eqref{eq:gradcond}.
...
result]\label{thm:ss}
Let Assumptions~\ref{ass:model}--\ref{ass:nondegen} and \ref{ass:path} hold,
let the four arm regressions use sieve dimension $K_{n}\asymp n^{a}$ (up to
logs), and suppose $s>(d+1)/2$. Let $R_{3}$ denote the third-order remainder
in \eqref{eq:expansion} at $\bar\gamma$ and impose
\begin{equation}
\textbf{(R3)}\qquad R_{3}=o_{p}(r_{n}^{*})\ \text{at }a=\tfrac{d}{2s+1},
\qquad\text{resp.}\qquad
R_{3}=o_{p}\big(\sqrt{K_{n}^{1/d}/n}\big)\ \text{for (ii)}.
\label{eq:R3cond}
\end{equation}
Then:
\begin{enumerate}[leftmargin=2em,label=(\roman*)]
\item (\emph{Rate.}) At $a=d/(2s+1)$,
$\hat\Th_{SS}-\Th_{0}=O_{p}(r_{n}^{*})$, $r_{n}^{*}=n^{-s/(2s+1)}$,
which by Theorem~\ref{thm:plugin}(ii) is minimax-optimal.
\item (\emph{Centered CLT.}) If in addition
\begin{equation}
\frac{d}{2s+1}\;<\;a\;<\;\frac{d}{d+2},
\label{eq:ss-window}
\end{equation}
a window that is \emph{nonempty if and only if $s>(d+1)/2$}, then
...
\end{enumerate}
The same conclusions hold for the tuning-free implementation
$\hat\Th_{SS}^{\,\mathrm{jack}}$ of \eqref{eq:jackknife} under the
additional condition \eqref{eq:jackcond}; see Remark~\ref{rem:jackgap}.
\end{theorem}

\begin{remark}[\textbf{(R3)} is an assumption, not a
consequence]\label{rem:R3}
Condition \textbf{(R3)} is where the weak-smoothness claim actually rests,
and we have not verified it from primitive conditions. What can be said is
how restrictive it is under each candidate bound on the third shape
derivative. Substituting $\norm{\bar\Delta}_{\infty}\asymp\sqrt{K_{n}/n}$ and
$\norm{\bar\Delta}_{C^{1}}\asymp K_{n}^{1/2+1/d}n^{-1/2}$ at
$a=d/(2s+1)$:
\[
\begin{array}{lll}
R_{3}\lesssim\norm{\bar\Delta}_{C^{1}}^{3}
 &\text{(3 gradients)} &\Longrightarrow\ s>\tfrac{3(d+1)}{4},\\[2pt]
R_{3}\lesssim\norm{\bar\Delta}_{\infty}\norm{\bar\Delta}_{C^{1}}^{2}
 &\text{(2 gradients)} &\Longrightarrow\ s>\tfrac{3d+1}{4},\\[2pt]
R_{3}\lesssim\norm{\bar\Delta}_{\infty}^{2}\norm{\bar\Delta}_{C^{1}}
 &\text{(1 gradient)} &\Longrightarrow\ s>\tfrac{3d-1}{4}.
\end{array}
\]
Only the last is weaker than $(d+1)/2$ for $d=2$ ($1.25$ against $1.5$); the
crude bound $\norm{\bar\Delta}_{C^{1}}^{3}$ that v2 wrote into the theorem
gives $s>2.25$ at $d=2$, far more restrictive than the $s>1.5$ the theorem
advertises. Since $D^{2}\Th$ in \eqref{eq:D2}--\eqref{eq:Qg} carries exactly
one gradient across its two directions, the one-gradient pattern is the
plausible one, and the same conjecture would close the jackknife gap of
Remark~\ref{rem:jackgap}. Settling it requires computing $D^{3}\Th$, which we
have not done. Until then Theorem~\ref{thm:ss} is a conditional theorem: the
$s>(d+1)/2$ headline is the range on which \emph{every other} term in the
error budget is controlled, and \textbf{(R3)} is assumed on that range.
\end{remark}
...
$R_{3}$, assumed negligible by \textbf{(R3)} --- see Remark~\ref{rem:R3} ---
and, for the jackknife implementation, the fourth-order error of
Lemma~\ref{lem:jackknife}. Ratio consistency of $\hat\sigma_{n}$ follows
from Proposition~\ref{prop:var} because the correction changes the estimator
by $o_{p}(\sigma_{n}/\sqrt n)$.
\end{proof}
...
removed by undersmoothing, not by debiasing. Consequently, provided the
\emph{integrated} cross-corner functional does not cancel ---
i.e.\ $\abs{\int_{\Corner}\varrho_{gh}\,w/(\Ng\Nh\abs{\sin\omega})
\dd\Haus^{d-2}}\geq\underline c>0$, the cross-term half of
Assumption~\ref{ass:nondegen}(c), which pointwise nonzero
$\overline{\rho\sigma_{S}\sigma_{Y}}$ does \emph{not} imply ---
...
stochastic and bias parts, of orders $\abs{\varrho_{gh}}K_{n}/n$ and
$K_{n}^{-2s/d}$ by \eqref{eq:pointwise}. That
...
and $\varepsilon_{n}^{-1}\norm{\hat\gamma_{-\kappa}-\gamma_{0}}_{\infty}
=o_{p}(1)$; (c) (\emph{boundary stability}) for both $\varphi\in\{g,h\}$ the
estimated level sets satisfy Assumption~\ref{ass:geometry}(a)--(b) with
constants bounded away from zero w.p.\ $\to1$, and
\[
d_{H}\big(\{\hat\varphi_{-\kappa}=0\},\{\varphi_{0}=0\}\big)=o_{p}(1),
\qquad
\mathrm{Leb}\big(\{\hat\varphi_{-\kappa}\geq0\}\triangle
\{\varphi_{0}\geq0\}\big)=o_{p}(1),
\]
with $d_{H}$ the Hausdorff distance between sets; equivalently, the two
surfaces are eventually normal graphs over one another with sup-norm
$o_{p}(1)$;
(d) the product rate
...
$\liminf\sigma_{n}^{2}/K_{n}^{1/d}>0$ being supplied by
...
two parallel segments $10^{-10}$ apart already fail it. Only a metric that
sees \emph{proximity} (Hausdorff distance, normal-graph convergence) or that
measures \emph{regions} rather than surfaces will do. v2 also stated (c) for
$g$ only. Condition (f) in v2 read
...
Under Assumptions~\ref{ass:model}--\ref{ass:nondegen} and \ref{ass:dml},
\[
...
because (e) forces the representer's approximation bias below
...
+D^{2}\Th[e_{B},e_{B}]\}\asymp K_{n}^{1+1/d}/n$ --- worse than the plug-in's,
and with the opposite sign. Theorem ``DD'' of v1--v2 is therefore false as
stated. Appendix~\ref{app:numeric}, item~7, exhibits both facts numerically
on the lens design.
\end{remark}
...
computation gives
$D_{A}\Psi[e_{A}]=\tfrac12D\Th_{0}[e_{A}]+\tfrac12D^{2}\Th[e_{A},e_{B}]$ and
...
a conditionally-mean-zero score. We therefore use three folds.
...
bands and the Gram matrix; and set
\begin{equation}
\hat\Th_{DD}^{(\pi)}
:=\Psi(\hat\gamma^{A},\hat\gamma^{B})
+\frac{3}{n}\sum_{i\in C}\sum_{o\in\{S,Y\}}\sum_{j\in\{A,B\}}
\hat v^{*j}_{o}(X_{i},W_{i})\big(O_{i}-\hat\mu^{j}_{o}(X_{i},W_{i})\big).
\label{eq:dd}
\end{equation}
...
$A\cup B$-measurable while the score runs over $C$, the score is
conditionally mean zero and its conditional expectation is
...
Suppose Assumption~\ref{ass:dml} holds for the ML first stages with (f)
replaced by its third-order half, and in addition: (i)
Assumption~\ref{ass:path} holds along $\bar\gamma+t\Delta$,
$\abs{t}\leq\tfrac12$, w.p.\ $\to1$; (ii)
$\norm{\hat\gamma^{j}-\gamma_{0}}_{C^{1}}=o_{p}(1)$ and $\hat\gamma^{A}$,
$\hat\gamma^{B}$ are independent; (iii) the third-order margin/corner
remainders and the fourth-order jackknife error of
Lemma~\ref{lem:jackknife} are $o_{p}(\sqrt{K_{n}^{1/d}/n})$; (iv)
$D^{2}\Th[e_{A},e_{B}]=o_{p}(\sqrt{K_{n}^{1/d}/n})$. Then for
$\hat\Th_{DD}$ of Definition~\ref{def:dd},
\[
\frac{\sqrt n\,(\hat\Th_{DD}-\Th_{0})}{\hat\sigma_{n}}
\overset{d}{\longrightarrow}\mathcal N(0,1),
\]
with $\hat\sigma_{n}$ from \eqref{eq:two-band-var} on cross-fitted residuals.
The gain over Theorem~\ref{thm:dml} is that the quadratic correction removes
the own-half second-order diagonal of the ML errors --- margins \emph{and}
corner --- so the smallness requirement on the learners drops from
Assumption~\ref{ass:dml}(f)'s second-order condition to (iii)--(iv). Note
...
again. The statement is conditional on (iii)--(iv) in the same sense as
...
the conditional mean-zero property of the fold-$C$ score. The leading term is
$\tfrac13\sum_{\pi}\tfrac3n\sum_{i\in C_{\pi}}v^{*}_{K_{n}}\epsilon_{i}
=\tfrac1n\sum_{i}v^{*}_{K_{n}}\epsilon_{i}$, whose CLT is
Assumption~\ref{ass:dml}(g); the remaining terms are (iii), (iv), the
representer's own approximation and estimation error
(Assumption~\ref{ass:dml}(d),(d$'$),(e)), and the out-of-span remainder
$R_{5}$.
\end{proof}
...
\subsection{Direct test of the cross-half off-diagonal order}
\label{sec:offdiag}

Table~\ref{tab:mc-main} contains, without any further computation, a
parameter-free test of the off-diagonal order derived in
Appendix~\ref{app:proofs}, \S B.2 --- the one input to
Theorem~\ref{thm:ss}(ii)'s window \eqref{eq:ss-window} that is otherwise
untested.
...
discriminates rather than merely fits.

Two corollaries. First, this \emph{completely} explains the SS standard
...
$0.78$--$0.96$ is thus \emph{entirely} the variance of the quadratic
...
Second, the finite-sample deficit is the visible shadow of a condition in
...
\eqref{eq:ss-window}. At the design's fixed $K=25$ the factor is
...
$K_{n}^{(d+2)/d}/n$ predicts, and that it accounts for the whole of the SS
...
positive on every design, a term $-\tfrac12D^{2}\Th[e,e]$ of the same sign as
that bias predicts an over-correction into negative territory, which is what
...
subject to (L1)--(L3); the cross-half off-diagonal order
$n^{-1}K_{n}^{(d+3)/(2d)}$ of Appendix~\ref{app:proofs}, \S B.2 --- which
sets the upper edge of the SS window \eqref{eq:ss-window} --- is confirmed by
the parameter-free test of Section~\ref{sec:offdiag} on the designs that
satisfy the assumptions and refuted on those that do not; the asymptotic
rate claims of
...
not written. Four items are not merely mechanical, and each is stated as an
assumption above rather than derived:
\begin{enumerate}[leftmargin=2.2em,label=(\arabic*)]
\item \textbf{The third shape derivative.} Condition \textbf{(R3)} of
Theorem~\ref{thm:ss}, the third-order half of
Assumption~\ref{ass:dml}(f), and condition (iii) of Theorem~\ref{thm:dd} all
require a bound on $D^{3}\Th$ that we have not computed. Under the crude
bound $\norm{\bar\Delta}_{C^{1}}^{3}$ the SS smoothness range shrinks from
$s>(d+1)/2$ to $s>3(d+1)/4$ (Remark~\ref{rem:R3}). The same computation
would settle the jackknife gap of Remark~\ref{rem:jackgap}.
\item \textbf{Uniform geometry along the path.}
Assumption~\ref{ass:path}(c)--(d) --- that the estimated level sets remain
uniformly regular and transversal along $\bar\gamma+t\Delta$ --- is assumed.
For sieve first stages it should follow from
$\norm{\hat\gamma-\gamma_{0}}_{C^{1}}=o_{p}(1)$ and the implicit function
theorem, uniformly; we have not written that argument, and it fails outright
for non-smooth learners (Remark~\ref{rem:tree-jack}).
\item \textbf{Band and Gram control.} Proposition~\ref{prop:var}(b)--(c) and
Assumption~\ref{ass:dml}(b),(d$'$) require an empirical-process argument for
a $K_{n}$-dimensional band average and for $\hat v^{*}$ in the
standard-deviation norm. This is the standard but nontrivial part of a sieve
variance proof; we assume it.
\item \textbf{Attainment of the margin bound.} Remark~\ref{rem:cancel} shows
the leading margin diagonal bound $K_{n}^{1+1/d}/n$ is attained only under
mesh resonance. Its exact order for curved margins, general $d$, and
heteroskedastic errors is open, and settling it would sharpen (or weaken)
the $s\geq d$ in Theorem~\ref{thm:plugin}(i).
\end{enumerate}
The corner parts of the argument, by contrast, require only the
transversality bound, the dimension count $\dim\Corner=d-2$, and
Assumption~\ref{ass:nondegen}(c) for the lower-bound direction.
...
\medskip
\noindent The scripts for items 1--7 sit alongside this document:
\begin{center}
\texttt{verify\_D2\_theorem.py},\quad \texttt{verify\_polarization.py},\\
\texttt{verify\_mesh\_resonance.py},\quad \texttt{verify\_dd\_score.py}.
\end{center}
