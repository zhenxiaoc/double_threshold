...
% ---------------------------------------------------------------------------
% Estimation and Inference on Two-Threshold Value Functionals -- VERSION 5
% ---------------------------------------------------------------------------
...
%
% v5 changes (see "Revision note (v5)" after the abstract):
%   * removes the invalid C1-continuity claim for the second derivative
%   * separates direct remainder assumptions from optional C2 sufficient bounds
%   * strengthens residual conditions for Riesz-variance consistency
%   * makes every stochastic decomposition arm-specific
%   * separates population-Gram linear terms from empirical-LS remainders
%   * defines and assumes consistency of the actual composite-score DD variance
%   * separates oracle, evaluation-point, and jackknife corner centering
% ---------------------------------------------------------------------------
...
we show that the margin bound, unlike the corner bound, is exhibited as
...
empirical-measure case, impose the Lyapunov condition
...
with $n^{-1}\sum_i\{(\hat v^{*}_{S,i})^{2}
+(\hat v^{*}_{Y,i})^{2}\}=O_{p}(\sigma_{n}^{2})$. Unweighted residual
...
$o(\mathrm{se}_{n})$, which under Assumption~\ref{ass:path}(d)--(e) is ensured by its
own sieve window:
\begin{equation}
...
Assumption~\ref{ass:nondegen}(c2). The oracle corner statistic omits the bias
...
and the foldwise empirical second moment of the feasible representer is
$O_p(\sigma_n^2)$;
...
Under Assumptions~\ref{ass:model}--\ref{ass:rem} and
\ref{ass:dml},
\[
...
the equicontinuity term of Assumption~\ref{ass:rem}(b), already assumed
...
\label{eq:dd-var}
\end{equation}
Suppose Assumptions~\ref{ass:model}--\ref{ass:rem} and
...
$D^{2}\Th[e_{A},e_{B}]=o_{p}(\sqrt{K_{n}^{1/d}/n})$; and (v)
\[
...
\overset{d}{\longrightarrow}\mathcal N(0,1),
\]
Condition (v) is a genuine additional condition on the composite
...
again. The statement is conditional on (iii)--(v) in the same sense as
...
composite representer by its target; the replacement error is controlled by
the composite versions of Assumption~\ref{ass:dml}(d),(d$'$),(e). Its CLT is
Assumption~\ref{ass:dml}(g); the remaining terms are (iii), (iv), the
out-of-span remainder $R_{5}$, and the empirical-measure term. Condition (v)
and Slutsky's theorem give studentization by \eqref{eq:dd-var}.
\end{proof}
...
upper bound unless Assumption~\ref{ass:nondegen}(c) is imposed.
...
the ordinary case. Ratio consistency of the actual composite-score variance
\eqref{eq:dd-var} is condition~(v) of the theorem. No equivalence with
\eqref{eq:two-band-var} is asserted.
\end{enumerate}
