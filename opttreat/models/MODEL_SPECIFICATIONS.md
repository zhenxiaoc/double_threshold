...
CCG `Model1` through `Model15` are implemented by one formula table in
`ccg_formula_model.py`; `TaylorExpansionModel` is the separate Taylor-style
model family.
...
| `TaylorExpansionModel` | configurable | `support0^p` | `support^p` | `0.5` | depends on `expansion` |
...
Observed covariates are drawn from `[-0.2, 1.2]^p`, and target covariates are
drawn from `[0, 1]^p`.
