...
def test_registered_models_generate_data() -> None:
    for model in [Model1(), Model4(), Model8(), Model15(), TaylorExpansionModel(K=3, p=3)]:
...
def test_model_registry_uses_two_model_families() -> None:
    assert isinstance(get_model("Model4"), CCGFormulaModel)
    assert isinstance(TaylorExpansionModel(K=3, p=3, expansion="tan2"), TaylorExpansionModel)
...
def test_ccg_models_are_registered_as_formula_models() -> None:
    factories = [
        Model1,
        Model2,
        Model3,
    ...
    for index, factory in enumerate(factories, start=1):
        model = factory()
        assert isinstance(model, CCGFormulaModel)
        assert model.name == f"Model{index}"
        assert isinstance(get_model(f"Model{index}"), CCGFormulaModel)


def test_explicit_taylor_tan2_special_case_returns_constant_effect_vectors() -> None:
    model = TaylorExpansionModel(K=3, p=3, expansion="tan2")
...
def test_taylor_model_true_effect_shape() -> None:
    model = TaylorExpansionModel(K=4, p=4, expansion="hyperbolic")
