...
from .random_sampler import random_sample_joint
...
def build_feature_map_from_options(
    ...
    2. Random feature generators via rfg_type:
        Required keys:
            - "rfg_type": {"iid_sphere", "quasi_sphere", "fabr"}
    ...
    activation = get_activation(options.get("activation", "sigmoid"))
    ...
    if rfg_type == "iid_sphere":
        ...
        if weight_dist is None:
            raise ValueError("rfg_type='fabr' requires 'weight_distribution'.")
        if joint and bias_dist is None:
            raise ValueError("rfg_type='fabr' with joint=True requires 'bias_distribution'.")
