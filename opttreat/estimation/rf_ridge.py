"""Random-feature ridge first-stage estimator."""
...
class RFRidgeEstimator(Estimator):
    """
    Random-feature ridge estimator of h(x) = mu_t(x) - mu_c(x).

    - Fits separate ridge regressions on random features for treated and control.
    - Can share one common feature map or use group-specific random features.

    Attributes:
    name:
    options:
        - "rfg_type",       (required)     | "iid_sphere", "quasi_sphere", or "fabr" 
        - "activation",     (required)     | "relu", "sigmoid", "exp", "tanh", "cos" (default)
    ...
    def fit(self, parsed_data: Dict[str, np.ndarray]) -> EstimatorOutput:
        ...
        alpha = float(self.options.get("alpha", 1e-3))
        self.alpha_ = alpha
        ...
        # -----------------------------------------------------------
        # Fit sklearn KernelRidge with linear kernel in RF space
        # -----------------------------------------------------------
        krr_t = Ridge(alpha=alpha, fit_intercept=False).fit(Psi_t, Y_t)
        krr_c = Ridge(alpha=alpha, fit_intercept=False).fit(Psi_c, Y_c)
        ...
        self.e_t_ = Y_t - krr_t.predict(Psi_t)
        self.e_c_ = Y_c - krr_c.predict(Psi_c)
        self.Psi_t_ = Psi_t
        self.Psi_c_ = Psi_c
